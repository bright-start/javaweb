...
public class Common {
	...
	public static CardType jugdeType(List<Card> list){
		...
		{//现在按相同数字最大出现次数
			...
			if( card_index.a[2].size()==1 &&  card_index.a[1].size()==1 &&len==5)
				return CardType.c32;
			if( card_index.a[2].size()==1 &&  card_index.a[0].size()==2 &&len==5)
				return CardType.c0;
			//4带2(单,双)
			if( card_index.a[3].size()==1 && card_index.a[0].size()==2  && len==6)
	...
	public static int checkCards(List<Card> c,List<Card>[] current){
		...
		//比较我的出牌类型
		System.out.println(cType.toString()+"======================我的牌型=========================");
		System.out.println(Common.jugdeType(currentlist).toString()+" -------- 电脑牌型---------------");
		...
		//炸弹牌炸非炸弹牌
		if(cType==CardType.c4 && Common.jugdeType(currentlist)!=CardType.c4)
		{
			if(c.size()==4)
				return 1;
		}
